...
app.use('/residency', express.static(path.join(__dirname, 'residency')))
app.use('/cs', express.static(path.join(__dirname, 'conflicting-spaces')))
...
function share (type, ws) {
  ...
  switch(type) {
    case 'history':
      msg.payload = state.history
      break
    case 'residency':
      msg.payload = state.residency
      break
  }
...
async function getBase (filePath) {
  const response = await fetch(`https://api.telegram.org/file/bot${process.env.TOKEN}/${filePath}`)
